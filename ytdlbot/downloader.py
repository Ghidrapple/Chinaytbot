...
def convert_to_mkv(resp: dict, bot_msg):
    default_type = ["video/x-flv", "video/webm"]
    if resp["status"]:
        # all_converted = []
        for path in resp["filepath"]:
            # if we can't guess file type, we assume it's video/mkv
            mime = getattr(filetype.guess(path), "mime", "video/mkv")
            if mime in default_type:
                if not can_convert_mkv(path, bot_msg.chat.id):
                    ...
                    break
                edit_text(bot_msg, f"{current_time()}: Converting {path.name} to mkv. Please wait.")
                new_file_path = path.with_suffix(".mkv")
                logging.info("Detected %s, converting to mkv...", mime)
...
def can_convert_mkv(video_path, uid):
    if not ENABLE_VIP:
        return True
...
def ytdl_download(url, tempdir, bm) -> dict:
    ...
    formats = [
        "bestvideo[ext=mkv]+bestaudio[ext=m4a]/bestvideo+bestaudio",
        "bestvideo[vcodec^=avc]+bestaudio[acodec^=mkva]/best[vcodec^=avc]/best",
        None
    ]
